...
def maestro(operaciones, planificaciones):
    ...
    #resolución
    problema.solve(lp.PULP_CBC_CMD(msg=False))
    quirofanos_act = [v.name.split('_')[1]+" "+v.name.split('_')[2] for v in problema.variables() if v.varValue > 0]

    return problema.objective.value(), quirofanos_act
...
num_iteraciones=0
iter_max=40 #ultima vez ha llegado hasta iteracion 23
fobj_dual = 40
...
for quiro in planificacion.keys():
    ...
    for j in planificacion[quiro]:
        monitor[j][0] += 1
        monitor[j].append(quiro)
'''

monitor = { i:[0] for i in operaciones.index.tolist()    }
for quiro in quirofanos_act:
    #check cuantas veces salen las operaciones
    for j in planificacion[quiro]:
        monitor[j][0] += 1
        monitor[j].append(quiro)

operaciones_asignadas=True
for i in monitor.keys():
    if monitor[i]==0:
        operaciones_asignadas=False


print(f'Estan todas las operaciones asignada: {operaciones_asignadas}')


#quiero entrar en las planificaciones y print aquellas para las que quiro en variables sea >1
for quir in quirofanos_act:
    print('\n')
    print(f'En el {quir} se planifican las siguientes operaciones {planificacion[quir]}')